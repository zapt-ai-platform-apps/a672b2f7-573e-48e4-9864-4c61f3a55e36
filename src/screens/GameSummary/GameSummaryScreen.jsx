...
import { getTotalPlayTime as calcTotalPlayTime, formatTime } from './utils/timeUtils';

function GameSummaryScreen() {
  const { playerData, goals, ourScore, opponentScore, includeGKPlaytime, resetGame } = useStateContext();
  const navigate = useNavigate();

  const getTotalPlayTime = (player) => calcTotalPlayTime(player, includeGKPlaytime);
    ...
    <div className="min-h-screen flex flex-col bg-white dark:bg-gray-900 text-gray-800 dark:text-white">
      <div className="p-8 flex-grow">
        ...
        <div className="flex space-x-4 mt-8">
          <button
            className="px-8 py-4 bg-brand-500 text-white text-lg rounded-md cursor-pointer hover:bg-brand-600 transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-brand-400"
