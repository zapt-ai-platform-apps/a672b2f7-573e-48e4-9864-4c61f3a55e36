...
export default function HeroSection({ onGetStarted }: HeroSectionProps): JSX.Element {
  return (
    <section className="flex flex-col items-center justify-center min-h-[70vh] py-16 px-4 text-center relative">
      <motion.div
        className="absolute top-20 right-20 w-40 h-40 rounded-full bg-blue-500/20 filter blur-[60px]"
        animate={{
          scale: [1, 1.2, 1],
          opacity: [0.3, 0.5, 0.3],
        }}
      ...
      <motion.div
        className="absolute bottom-20 left-20 w-60 h-60 rounded-full bg-purple-600/20 filter blur-[80px]"
        animate={{
          scale: [1, 1.3, 1],
          opacity: [0.2, 0.4, 0.2],
        }}
      ...
      >
        <img 
          ...
          className="mx-auto w-36 h-36 drop-shadow-[0_0_30px_rgba(59,130,246,0.7)]"
        />
        <motion.div 
      ...
      <motion.h1 
        className="text-5xl md:text-7xl font-bold mb-6 text-transparent bg-clip-text bg-gradient-to-r from-blue-200 via-indigo-300 to-purple-200 animate-text-shimmer tracking-tight"
      ...
      >
        ...
        <motion.a 
          href="#features" 
          className="group mt-4 sm:mt-0 px-6 py-3 text-white text-lg border border-white/30 rounded-xl hover:bg-white/10 transition-all duration-300 flex items-center gap-2 cursor-pointer backdrop-blur-sm"
      ...
      >
        <h3 className="text-xl text-blue-200 mb-4 font-semibold">Perfect for all teams</h3>
        <div className="flex flex-wrap justify-center gap-3">
          {[
            ...
            <motion.div 
              key={index} 
              className="px-4 py-2 bg-white/10 backdrop-blur-sm rounded-full text-white border border-white/10 shadow-lg flex items-center gap-2"
